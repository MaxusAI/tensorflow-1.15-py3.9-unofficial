...
Usage() {
  ...
  echo "    --no-deps                      Install TensorFlow wheel with --no-deps pip option"
}
...
BAZEL_INSTALLED=$(bazel version 2>/dev/null | grep "Build label" | cut -d' ' -f3 | grep -o '[0-9.]*')
...
if [[ -n "$BAZEL_EXPECTED" && "$BAZEL_INSTALLED" != "$BAZEL_EXPECTED" ]]; then
  ...
  echo Re-installing bazel $BAZEL_EXPECTED over $BAZEL_INSTALLED
  ...
  if [[ "$TARGETARCH" == "amd64" ]]; then
    ...
    # TF1-era Bazel binaries were not provided for ARM, so we must build from source.
    curl -fSsL -O https://github.com/bazelbuild/bazel/releases/download/$BAZEL_EXPECTED/bazel-$BAZEL_EXPECTED-dist.zip
    unzip bazel-$BAZEL_EXPECTED-dist.zip -d bazel
    if [[ $? -ne 0 ]]; then
      echo Failed to get bazel sources
      bazel_cleanup
      exit 1
    fi
    cd bazel
    pushd third_party/grpc
    patch -p0 < /opt/tensorflow/bazel_grpc.patch
    popd
    env SOURCE_DATE_EPOCH=1 EXTRA_BAZEL_ARGS="--host_javabase=@local_jdk//:jdk" bash ./compile.sh
    install output/bazel /usr/local/bin/
    if [[ $? -ne 0 ]]; then
      echo Failed to build bazel.
      cd ..
      bazel_cleanup
      exit 1
    fi
  ...
  bazel_cleanup
  echo "Successfully installed bazel $(bazel version 2>/dev/null | grep "Build label" | cut -d' ' -f3)"
  set -e
else
